# Safe hosts apply/remove for Duckgram only.
...
trap {
    ...
    if ($msg -match '(?i)access|denied|отказано|запрещен|Unauthorized') {
        $msg = "нет доступа к hosts — запустите bat от имени администратора (UAC)"
    }
