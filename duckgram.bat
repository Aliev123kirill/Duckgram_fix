...
set "MARKER=# duckgram"
set "MARKER_LEGACY=# telegram-web-fix"
set "CFG=%~dp0duckgram-ip.cfg"
...
echo.
set "UACVBS=%TEMP%\duckgram_elev_%RANDOM%.vbs"
...
set "HOSTS_ERR=%TEMP%\duckgram_hosts_err.log"
...
set "HOSTS_ERR=%TEMP%\duckgram_hosts_err.log"
del "!HOSTS_ERR!" >nul 2>&1
"%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -File "%PS_HOSTS%" -Action Apply -Ip "%TG_IP%" -DomainsFile "%DOMAINS%" -BackupDir "%BACKUP%" 1>nul 2>"!HOSTS_ERR!"
set "PS_EXIT=!errorlevel!"
...
set "HOSTS_ERR=%TEMP%\duckgram_hosts_err.log"
del "!HOSTS_ERR!" >nul 2>&1
"%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -File "%PS_HOSTS%" -Action Remove -DomainsFile "%DOMAINS%" 1>nul 2>"!HOSTS_ERR!"
set "PS_EXIT=!errorlevel!"
if not "!PS_EXIT!"=="0" exit /b 1
...
echo.
echo   !CYN!╔════════════════════════════════════════════════════════════════════════════════════════════╗!R!
echo   !CYN!║!R!                                       !GRN!DUCKGRAM!R!                                         !CYN!║!R!
...
set /a _bxp=51
call :pad92_len !_bxp!
echo   !GRY!^|!R!  Конфиг ............ !WHT!duckgram-ip.cfg!R!!_P92!!GRY!^|!R!
:st_after_ip
set /a _bxp=50
call :pad92_len !_bxp!
...
set /a _g=14 & set /a _t=34
call :pad92_len !_g!
set "_gsp=!_P92!"
call :pad92_len !_t!
set "_tr=!_P92!"
echo   !GRY!^|!R!   !YLW![6]!R!  !WHT!Авто-настройка!R!!_gsp!!DIM!- всё сам: поиск+запись+проверка!R!!_tr!!GRY!^|!R!
set /a _g=4 & set /a _t=41
call :pad92_len !_g!
set "_gsp=!_P92!"
call :pad92_len !_t!
...
if "%~1"=="ERR" exit /b 0
if "%~1"=="FOUND" echo   !GRN!  Сохранён в duckgram-ip.cfg: %~2!R!
...
if "!choice!"=="6" goto :auto_settings
...
call :line_info "Источник: Telegram, DoH, зеркала GitHub/GitLab/jsDelivr, локальная копия"
call :box_bottom
echo.
...
if "!UP_OK!"=="0" (
    ...
    call :line_fail "Не удалось получить cidr.txt — сайт Telegram, зеркала, lists\telegram-cidr-official.txt"
    call :line_info "Проверьте интернет; при блокировке РКН сработает локальная копия в lists"
)
call :wait
goto :menu
:up_no_script
...
:auto_settings
call :cls_ui
call :section "Авто-настройка (Duckgram): всё сам"
call :line_info "Сначала проверим, есть ли готовый список IP"
call :box_bottom
echo.
rem --- Шаг 0: если есть cfg с IP, можно сразу пробовать применить -> но лучше пройти полный цикл.
rem Проверяем, заполнен ли файл кандидатов.
set "CAND_COUNT=0"
if exist "%PS_UPDATE%" if exist "%~dp0lists\telegram-ip-candidates.txt" (
    for /f %%N in ('findstr /R /C:"[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*" "%~dp0lists\telegram-ip-candidates.txt" 2^>nul ^| find /c /v ""') do set "CAND_COUNT=%%N"
)
if !CAND_COUNT! gtr 0 (
    call :line_ok "Список IP найден: !CAND_COUNT! адресов — пропускаем обновление"
    goto :as_skip_update
)
echo.
call :line_info "Шаг 1/4: Список IP пуст — обновляем (cidr.txt с Telegram)"
call :box_bottom
echo.
if not exist "%PS_UPDATE%" goto :as_no_update
set "PS_CMD=%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%PS_UPDATE%""
call :run_ps_visible
set "AS_UP=!PS_EXIT!"
if "!AS_UP!" neq "0" (
    call :line_fail "Не удалось обновить список IP — но попробуем поиск по DNS"
    echo.
)
if "!AS_UP!"=="0" call :line_ok "Список IP обновлён"
:as_skip_update
echo.
call :line_info "Шаг 2/4: Поиск рабочего IP (TCP 443 + TLS)"
call :box_bottom
echo.
if not exist "%PS_DETECT%" goto :as_no_detect
set "SCAN_LOG=%TEMP%\duckgram-scan.log"
del "%SCAN_LOG%" 2>nul
...
if exist "%SCAN_LOG%" (
    ...
    set /a _bxp=84
    call :pad92_len !_bxp!
    echo   !GRY!^|!R!  !WHT!Отчёт скана!R!!_P92!!GRY!^|!R!
    echo   !GRY!├────────────────────────────────────────────────────────────────────────────────────────────┤!R!
    call :scan_show_log "%SCAN_LOG%"
    echo   !GRY!└────────────────────────────────────────────────────────────────────────────────────────────┘!R!
)
echo.
if defined TG_IP goto :as_ip_found
rem --- Не нашли IP. Если список не был обновлён на этом прогоне — пробуем обновить и ещё раз.
if "!AS_UP!"=="0" goto :as_no_ip
call :line_fail "IP не найден с текущим списком"
call :line_info "Попробую обновить список IP и повторить поиск (1 ретрай)..."
call :box_bottom
echo.
if not exist "%PS_UPDATE%" goto :as_no_ip
set "PS_CMD=%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%PS_UPDATE%""
call :run_ps_visible
set "AS_UP=!PS_EXIT!"
if "!AS_UP!" neq "0" (
    call :line_fail "Обновление не помогло"
    goto :as_no_ip
)
call :line_ok "Список обновлён, повторяю поиск..."
echo.
set "SCAN_LOG=%TEMP%\duckgram-scan.log"
del "%SCAN_LOG%" 2>nul
call :run_ps_scan_console
...
:as_no_ip
if defined TG_IP goto :as_ip_found
call :line_fail "IP не найден — сеть/VPN/фаервол блокируют доступ к DC Telegram"
call :wait
goto :menu
:as_ip_found
if not defined TG_IP goto :as_no_ip
call :line_ok "Найден рабочий IP: !TG_IP!"
echo.
call :line_info "Шаг 3/4: Запись в hosts"
call :box_bottom
echo.
call :apply_hosts
set "AS_EN=!errorlevel!"
if "!AS_EN!" neq "0" (
...
call :line_ok "hosts обновлён"
call :line_ok "DNS сброшен"
echo.
call :line_info "Шаг 4/4: Проверка hosts и доступность web.telegram.org"
call :box_bottom
echo.
if not exist "%PS_HOSTS%" goto :as_no_hosts
set "HOSTS_ERR=%TEMP%\duckgram_hosts_err.log"
del "!HOSTS_ERR!" >nul 2>&1
"%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -File "%PS_HOSTS%" -Action Verify -DomainsFile "%DOMAINS%" 1>nul 2>"!HOSTS_ERR!"
set "PS_EXIT=!errorlevel!"
if "!PS_EXIT!" neq "0" (
    call :line_fail "Проверка hosts: наши строки не найдены или повреждены"
    call :wait
    goto :menu
)
call :line_ok "hosts прошёл проверку — строки на месте"
set "CHECK_LOG=%TEMP%\duckgram_final_check.log"
del "%CHECK_LOG%" 2>nul
"%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command ^
    "$ip='!TG_IP!'; $hosts=@('web.telegram.org','kws2.web.telegram.org'); $ok=0; $fail=@(); " ^
    "foreach($h in $hosts){ try{ $c=[Net.Sockets.TcpClient]::new(); $iar=$c.BeginConnect($ip,443,$null,$null); " ^
    "if($iar.AsyncWaitHandle.WaitOne(3000)){ $c.EndConnect($iar); if($c.Connected){ " ^
    "$ssl=New-Object Net.Security.SslStream($c.GetStream(),$false); " ^
    "$ar=$ssl.BeginAuthenticateAsClient($h,$null,$null); " ^
    "if($ar.AsyncWaitHandle.WaitOne(8000)){ $ssl.EndAuthenticateAsClient($ar); " ^
    "if($ssl.IsAuthenticated){ $ok++; Write-Host ('  [OK] TLS {0} на {1}' -f $h,$ip) -Fore Green } " ^
    "else{ $fail+=$h } } else{ $fail+=$h } } else{ $fail+=$h } } else{ $fail+=$h } } " ^
    "catch{ $fail+=$h } } " ^
    "if($ok -eq $hosts.Count){ Write-Host '  [OK] Все хосты доступны и TLS проходит' -Fore Green; exit 0 } " ^
    "else{ Write-Host ('  [FAIL] Не прошли: {0}' -f ($fail -join ', ')) -Fore Red; exit 1 }" ^
    1>"%CHECK_LOG%" 2>&1
set "CHK_EXIT=!errorlevel!"
type "%CHECK_LOG%" 2>nul
echo.
if "!CHK_EXIT!" neq "0" (
    call :line_fail "Финальная проверка: web.telegram.org недоступен через найденный IP"
    call :wait
    goto :menu
)
call :line_ok "web.telegram.org и kws2.web.telegram.org — доступны, TLS OK"
echo.
call :banner_ok "АВТО-НАСТРОЙКА ЗАВЕРШЕНА УСПЕШНО!"
echo   !GRN!  Всё замечательно! Откройте web.telegram.org в браузере.!R!
echo.
call :wait
goto :menu
:as_no_update
...
if exist "!HOSTS_ERR!" (
    for /f "usebackq delims=" %%L in ("!HOSTS_ERR!") do (
        echo %%L | findstr /i /c:"tg-hosts:" >nul 2>&1
        if not errorlevel 1 (
            set "HOSTS_MSG=%%L"
...
if defined HOSTS_MSG (
    ...
    call :line_info "Нужен запуск от администратора; hosts не read-only; антивирус может блокировать запись"
)
exit /b 0
:disable
call :cls_ui
...
if "!DH_OK!"=="0" (
    ...
    if "!FIX_ON!"=="1" (
        ...
        call :line_info "Проверьте файл hosts вручную (маркер # duckgram)"
    ) else (
...
call :line_info "[2] снимает только блок с маркером # duckgram"
call :wait
goto :menu
:show
call :cls_ui
